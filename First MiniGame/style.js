var game,level,color=["PIK.png","rosneft.png","sber.png","AMD.png","Facebook.png"],water=[],w=[],currentLevel,clicked=[],transferring=false,t=false,size=1,sizechange=0.05,won=false,moves=0;
...
var levelArray = [ 0,0,1,1,2,2 ];
var levelPointer = 0;

window.onload = function() {
    game = document.getElementById("game");
    level = document.getElementById("level");
    OpenLevel(levelArray[levelPointer]);
}
    ...
    if (!won) {
        ...
        for (let i of testTubePosition[currentLevel]) {
            level.innerHTML += `<div class = "test-tube" style="top:${i[1]}px;left:calc(50vw + ${i[0]}px);transform:rotate(0deg);" onclick="Clicked(${d});">
                ${a[d][0]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:100px;'></div>":"<div class='colors' style = 'top:100px;'><img src="+a[d][0]+"></div>"}
                ${a[d][1]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:70px;'></div>":"<div class='colors' style = 'top:70px;'><img src="+a[d][1]+"></div>"}
                ${a[d][2]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:40px;'></div>":"<div class='colors' style = 'top:40px;'><img src="+a[d][2]+"></div>"}
                ${a[d][3]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:10px;'></div>":"<div class='colors' style = 'top:10px;'><img src="+a[d][3]+"></div>"}
            </div>`;
            d++;
        }
        ...
        level.innerHTML+=`<div id = "restart" class = "game-buttons" style='color:white' onclick = "Restart();"><img width="150" height="150" style="display: block; margin:auto" src="Replay.svg">Replay</div>`;
    }
}
    ...
    if (!transferring) {
        if (clicked.length == 0) {
            ...
            if (clicked[0]!=clicked[1]) {
                el.style.transition = "0.7s linear"; // move animation
                moves++;
...
function WaterDec(p,a,count) {
    ...
    setTimeout(function() {
        document.getElementsByClassName("test-tube")[a].innerHTML = `
            ${water[a][0]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:100px;'></div>":"<div class='colors' style = 'top:100px;'><img src="+water[a][0]+"></div>"}
            ${water[a][1]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:70px;'></div>":"<div class='colors' style = 'top:70px;'><img src="+water[a][1]+"></div>"}
            ${water[a][2]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:40px;'></div>":"<div class='colors' style = 'top:40px;'><img src="+water[a][2]+"></div>"}
            ${water[a][3]=="transparent"?"<div class='colors' style = 'background-color:transparent;top:10px;'></div>":"<div class='colors' style = 'top:10px;'><img src="+water[a][3]+"></div>"}`;
    },1050);
    console.log(document.getElementsByClassName("test-tube")[a].innerHTML);
}
...
function WaterInc(p,q,b,count) {
    ...
    // console.log(p); using tube
    // console.log(q); target
    // console.log(b); index of fulling tube
    // console.log(count); number of blocks in tube
    
    let tmpStr = ("<img src="+p[0]+">").repeat(count);

    document.getElementsByClassName("test-tube")[b].innerHTML += `<div id = "colorful-bg" style = "top:calc(10px + ${q[1]*30}px);">${tmpStr}</div>`;
    // console.log(document.getElementsByClassName("test-tube")[b].innerHTML);
    setTimeout(function() {
...
function Won() {
    ...
    // TODO need some reinforcment wee-wee HORAY!!!1 shit
    // else level.innerHTML = `<div id="won">YOU WON</div><div id = "restart" class = "game-buttons" onclick = "Restart();">RESTART</div><div id = "home" class = "game-buttons" onclick = "ShowMenu();">HOME</div>`;
    let heightAvg=0;
    let widthAvg=0;

    for(let i=0;i<testTubePosition[currentLevel].length;i++){
        heightAvg+=testTubePosition[currentLevel][i][1];
        widthAvg+=testTubePosition[currentLevel][i][0];
        // document.getElementsByClassName("test-tube")[i].zIndex="0";
    }
    // console.log(heightAvg,widthAvg);
    heightAvg/=testTubePosition[currentLevel].length;
    widthAvg/=testTubePosition[currentLevel].length;
    for(let i=0;i<testTubePosition[currentLevel].length;i++){
        let el = document.getElementsByClassName("test-tube")[i];
        el.style.top = `calc(${heightAvg}px)`;
        el.style.left = `calc(50vw + ${widthAvg}px)`;
    }
    let glass = document.getElementsByClassName("test-tube")[testTubePosition[currentLevel].length-1];
    glass.style.transform = "scale(2)";
    glass.innerHTML="";
    
    glass.classList.add('animated-tube');
...
window.HideRules = function() {
    ...
    if(levelPointer<levelArray.length-1) OpenLevel(levelArray[++levelPointer]);
    else level.innerHTML="";
}